  ...
  try{
    await fetch("/api/public-order", {
      ...
      body: JSON.stringify({
        ...
        monto: total, items: cart.map(i => ({ nombre: i.nombre, qty: i.qty, dedicatoria: i.dedicatoria || undefined, img: i.img || undefined }))
      })
    });
...
async function pagarConMercadoPago(){
  ...
  try{
    ...
    const items = cart.map(i => {
      ...
      return { title, qty: i.qty, price: Math.round(i.precio * factor * 100) / 100, dedicatoria: i.dedicatoria || undefined, img: i.img || undefined };
    });
    const zonaSel = getZonaSeleccionada();
